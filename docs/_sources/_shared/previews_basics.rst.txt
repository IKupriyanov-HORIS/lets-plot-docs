.. |1d_distributions-nbviewer| image:: /examples/basics/previews/1d_distributions.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/1d_distributions.ipynb

.. |2d_distributions-nbviewer| image:: /examples/basics/previews/2d_distributions.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/2d_distributions.ipynb

.. |3_variables-nbviewer| image:: /examples/basics/previews/3_variables.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/3_variables.ipynb

.. |brewer_scale-nbviewer| image:: /examples/basics/previews/brewer_scale.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/brewer_scale.ipynb

.. |cartesian_coordinates-nbviewer| image:: /examples/basics/previews/cartesian_coordinates.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/cartesian_coordinates.ipynb

.. |comparisons-nbviewer| image:: /examples/basics/previews/comparisons.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/comparisons.ipynb

.. |continuous_scales-nbviewer| image:: /examples/basics/previews/continuous_scales.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/continuous_scales.ipynb

.. |correlation-nbviewer| image:: /examples/basics/previews/correlation.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/correlation.ipynb

.. |correlation_plot-nbviewer| image:: /examples/basics/previews/correlation_plot.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/correlation_plot.ipynb

.. |datetime_scale-nbviewer| image:: /examples/basics/previews/datetime_scale.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/datetime_scale.ipynb

.. |default_theme-nbviewer| image:: /examples/basics/previews/default_theme.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/default_theme.ipynb

.. |discrete_scales-nbviewer| image:: /examples/basics/previews/discrete_scales.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/discrete_scales.ipynb

.. |dodge_position-nbviewer| image:: /examples/basics/previews/dodge_position.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/dodge_position.ipynb

.. |facet_grid-nbviewer| image:: /examples/basics/previews/facet_grid.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/facet_grid.ipynb

.. |facet_wrapping-nbviewer| image:: /examples/basics/previews/facet_wrapping.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/facet_wrapping.ipynb

.. |fixed_coordinates-nbviewer| image:: /examples/basics/previews/fixed_coordinates.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/fixed_coordinates.ipynb

.. |functions-nbviewer| image:: /examples/basics/previews/functions.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/functions.ipynb

.. |general_purpose_stats-nbviewer| image:: /examples/basics/previews/general_purpose_stats.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/general_purpose_stats.ipynb

.. |geom_area-nbviewer| image:: /examples/basics/previews/geom_area.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_area.ipynb

.. |geom_area_bin-nbviewer| image:: /examples/basics/previews/geom_area_bin.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_area_bin.ipynb

.. |geom_bar-nbviewer| image:: /examples/basics/previews/geom_bar.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_bar.ipynb

.. |geom_bar_identity-nbviewer| image:: /examples/basics/previews/geom_bar_identity.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_bar_identity.ipynb

.. |geom_bin2d-nbviewer| image:: /examples/basics/previews/geom_bin2d.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_bin2d.ipynb

.. |geom_boxplot-nbviewer| image:: /examples/basics/previews/geom_boxplot.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_boxplot.ipynb

.. |geom_contour-nbviewer| image:: /examples/basics/previews/geom_contour.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_contour.ipynb

.. |geom_contourf-nbviewer| image:: /examples/basics/previews/geom_contourf.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_contourf.ipynb

.. |geom_crossbar-nbviewer| image:: /examples/basics/previews/geom_crossbar.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_crossbar.ipynb

.. |geom_density-nbviewer| image:: /examples/basics/previews/geom_density.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_density.ipynb

.. |geom_density2d-nbviewer| image:: /examples/basics/previews/geom_density2d.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_density2d.ipynb

.. |geom_density2df-nbviewer| image:: /examples/basics/previews/geom_density2df.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_density2df.ipynb

.. |geom_errorbar-nbviewer| image:: /examples/basics/previews/geom_errorbar.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_errorbar.ipynb

.. |geom_freqpoly-nbviewer| image:: /examples/basics/previews/geom_freqpoly.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_freqpoly.ipynb

.. |geom_histogram-nbviewer| image:: /examples/basics/previews/geom_histogram.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_histogram.ipynb

.. |geom_jitter_cc-nbviewer| image:: /examples/basics/previews/geom_jitter_cc.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_jitter_cc.ipynb

.. |geom_jitter_dd-nbviewer| image:: /examples/basics/previews/geom_jitter_dd.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_jitter_dd.ipynb

.. |geom_line-nbviewer| image:: /examples/basics/previews/geom_line.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_line.ipynb

.. |geom_linerange-nbviewer| image:: /examples/basics/previews/geom_linerange.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_linerange.ipynb

.. |geom_map-nbviewer| image:: /examples/basics/previews/geom_map.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_map.ipynb

.. |geom_path-nbviewer| image:: /examples/basics/previews/geom_path.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_path.ipynb

.. |geom_point-nbviewer| image:: /examples/basics/previews/geom_point.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_point.ipynb

.. |geom_pointrange-nbviewer| image:: /examples/basics/previews/geom_pointrange.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_pointrange.ipynb

.. |geom_polygon-nbviewer| image:: /examples/basics/previews/geom_polygon.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_polygon.ipynb

.. |geom_raster-nbviewer| image:: /examples/basics/previews/geom_raster.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_raster.ipynb

.. |geom_rect-nbviewer| image:: /examples/basics/previews/geom_rect.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_rect.ipynb

.. |geom_ribbon-nbviewer| image:: /examples/basics/previews/geom_ribbon.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_ribbon.ipynb

.. |geom_segment-nbviewer| image:: /examples/basics/previews/geom_segment.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_segment.ipynb

.. |geom_smooth-nbviewer| image:: /examples/basics/previews/geom_smooth.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_smooth.ipynb

.. |geom_step-nbviewer| image:: /examples/basics/previews/geom_step.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_step.ipynb

.. |geom_text-nbviewer| image:: /examples/basics/previews/geom_text.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_text.ipynb

.. |geom_tile-nbviewer| image:: /examples/basics/previews/geom_tile.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/geom_tile.ipynb

.. |gradient_scale-nbviewer| image:: /examples/basics/previews/gradient_scale.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/gradient_scale.ipynb

.. |gradient2_scale-nbviewer| image:: /examples/basics/previews/gradient2_scale.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/gradient2_scale.ipynb

.. |graph_building-nbviewer| image:: /examples/basics/previews/graph_building.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/graph_building.ipynb

.. |grey_scale-nbviewer| image:: /examples/basics/previews/grey_scale.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/grey_scale.ipynb

.. |guide_colorbar-nbviewer| image:: /examples/basics/previews/guide_colorbar.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/guide_colorbar.ipynb

.. |guide_legend-nbviewer| image:: /examples/basics/previews/guide_legend.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/guide_legend.ipynb

.. |hue_scale-nbviewer| image:: /examples/basics/previews/hue_scale.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/hue_scale.ipynb

.. |identity_scales-nbviewer| image:: /examples/basics/previews/identity_scales.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/identity_scales.ipynb

.. |image_matrix-nbviewer| image:: /examples/basics/previews/image_matrix.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/image_matrix.ipynb

.. |jitter_position-nbviewer| image:: /examples/basics/previews/jitter_position.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/jitter_position.ipynb

.. |jitterdodge_position-nbviewer| image:: /examples/basics/previews/jitterdodge_position.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/jitterdodge_position.ipynb

.. |labs-nbviewer| image:: /examples/basics/previews/labs.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/labs.ipynb

.. |legend_place-nbviewer| image:: /examples/basics/previews/legend_place.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/legend_place.ipynb

.. |log10_scale-nbviewer| image:: /examples/basics/previews/log10_scale.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/log10_scale.ipynb

.. |manual_scales-nbviewer| image:: /examples/basics/previews/manual_scales.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/manual_scales.ipynb

.. |map_coordinates-nbviewer| image:: /examples/basics/previews/map_coordinates.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/map_coordinates.ipynb

.. |maps_and_geocoding-nbviewer| image:: /examples/basics/previews/maps_and_geocoding.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/maps_and_geocoding.ipynb

.. |nudge_position-nbviewer| image:: /examples/basics/previews/nudge_position.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/nudge_position.ipynb

.. |plot_title-nbviewer| image:: /examples/basics/previews/plot_title.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/plot_title.ipynb

.. |reversed_scale-nbviewer| image:: /examples/basics/previews/reversed_scale.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/reversed_scale.ipynb

.. |scale_way-nbviewer| image:: /examples/basics/previews/scale_way.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/scale_way.ipynb

.. |shape_manual_scale-nbviewer| image:: /examples/basics/previews/shape_manual_scale.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/shape_manual_scale.ipynb

.. |shape_scale-nbviewer| image:: /examples/basics/previews/shape_scale.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/shape_scale.ipynb

.. |size_area_scale-nbviewer| image:: /examples/basics/previews/size_area_scale.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/size_area_scale.ipynb

.. |size_scale-nbviewer| image:: /examples/basics/previews/size_scale.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/size_scale.ipynb

.. |with_clipping-nbviewer| image:: /examples/basics/previews/with_clipping.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/with_clipping.ipynb

.. |without_clipping-nbviewer| image:: /examples/basics/previews/without_clipping.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/without_clipping.ipynb

.. |xlab_ylab-nbviewer| image:: /examples/basics/previews/xlab_ylab.png
    :target: https://nbviewer.jupyter.org/github/ASmirnov-HORIS/lets-plot-docs/blob/dev/source/examples/basics/xlab_ylab.ipynb